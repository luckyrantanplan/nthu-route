...
Multisource_multisink_mazeroute::Multisource_multisink_mazeroute(Construct_2d_tree& construct_2d_tree) :
    ...
    for (int i = 0; i < rr_map.get_gridx(); ++i) {
        for (int j = 0; j < rr_map.get_gridy(); ++j) {
            mmm_map[i][j].setCoor(construct_2d_tree.coor_array[i][j]);
        }
    }
...
bool Multisource_multisink_mazeroute::mm_maze_route_p2(Two_pin_element_2d &ielement, double bound_cost, int bound_distance, int bound_via_num, Coordinate_2d& start, Coordinate_2d& end) {
    ...
    while (!pqueue.empty()) {
        ...
        if (cur_pos.parent != nullptr) {
            pre_dir = Coordinate_2d::get_direction_simple(cur_pos.getCoor(), cur_pos.parent->getCoor());
        ...
        for (int i = 3; i >= 0; --i) {
            if ((i == 3 && cur_pos.getCoor().x >= gridxMinusOne) || (i == 2 && cur_pos->coor.x <= 0) || (i == 1 && cur_pos->coor.y <= 0) || (i == 0 && cur_pos->coor.y >= gridyMinusOne))
                continue;
            else {
                next_pos = &mmm_map[cur_pos.getCoor().x + construct_2d_tree.dir_array[i][0]][cur_pos.getCoor().y + construct_2d_tree.dir_array[i][1]];
                ...
                if (next_pos != cur_pos.parent && next_pos->walkableID == visit_counter) {
                    ...
                    CacheEdge& edge = construct_2d_tree.cache->edge(cur_pos.getCoor().x, cur_pos.getCoor().y, dirType);
